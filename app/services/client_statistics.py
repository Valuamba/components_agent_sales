...
class ClientStatisticsService:
    ...
    def avg_interval_between_purchases(purchase_history):
        ...
        intervals = [(data[i]['requisition_created'] - data[i - 1]['requisition_created']).days for i in
                     range(1, len(data))]
    ...
    def summarize_client_metrics(purchase_history):
        ...
        average_interval, average_interval_months = ClientStatisticsService.avg_interval_between_purchases(
            purchase_history)
        ...
        client_metrics.append(
            f"Most common product: {products_analysis['most_common_product'][0]} (Purchased {products_analysis['most_common_product'][1]} times)")
        client_metrics.append(
            f"Most common brand: {products_analysis['most_common_brand'][0]} (Purchased {products_analysis['most_common_brand'][1]} times)")
        ...
        for client_id, metrics in client_loyalty_metrics.items():
            client_metrics.append(
                f"Client ID {client_id}: Duration of Business Relationship: {metrics['Duration Years']:.2f} years ({metrics['Duration Days']} days)")
